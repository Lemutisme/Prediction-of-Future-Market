...
inSample=[datenum('01/01/2005'),datenum('12/31/2018')];
outSample=[datenum('01/01/2019'),datenum('04/14/2020')];
...
ind=(indOutSample1:indOutSample2);figure(2); clf(2); X=[d.Open(ind), d.High(ind), d.Low(ind), d.Close(ind)];  highlow(X,'b'); hold on; plot(HH(ind),'g'); plot(HH(ind)*(1-S),'--g'); plot(LL(ind),'c'); plot(LL(ind)*(1+S),'--c'); plot((0.5./trades(ind)).*HH(ind),'.r');plot((0.5./trades(ind)).*LL(ind),'.r'); plot((0.5./trades(ind)).*HH(ind)*(1-S),'.r'); plot((0.5./trades(ind)).*LL(ind)*(1+S),'.r'); plot((trades(ind))*4+2,'--r'); hold off;

ind=(indOutSample1:indOutSample2);figure(3); clf(3); T=datetime(d.numTime(ind),'ConvertFrom','datenum'); X=[d.Open(ind), d.High(ind), d.Low(ind), d.Close(ind)]; TT = array2timetable(X,'RowTimes',T,'VariableNames',{'open','high','low','close'}); highlow(TT,'b'); hold on; plot(T,HH(ind),'g'); plot(T,HH(ind)*(1-S),'--g'); plot(T,LL(ind),'c'); plot(T,LL(ind)*(1+S),'--c'); plot(T,(0.5./trades(ind)).*HH(ind),'.r');plot(T,(0.5./trades(ind)).*LL(ind),'.r'); plot(T,(0.5./trades(ind)).*HH(ind)*(1-S),'.r'); plot(T,(0.5./trades(ind)).*LL(ind)*(1+S),'.r'); plot(T,(trades(ind))*4+2,'--r'); hold off;

[a,x1]=(max(-resultInSample(:,:,1)./resultInSample(:,:,2).*(resultInSample(:,:,4)<300)));
[a,x2]=max(a); 
x1=x1(x2);
disp([Length(x1),StopPct(x2)]);


[a,x1]=(max(-resultInSample(:,:,1)./resultInSample(:,:,2))); % finds maximum performance for each StopPct, with corresponding indexes
[a,x2]=max(a); %find maximim performance for all results and corresponding index for StopPct
x1=x1(x2); %gives the index for Length that maximizes performance for StopPct

l=Length(x1); s=StopPct(x2); %assign the optimzal parameters and output the result
disp(['optimal parameters: Length = ' num2str(l) ', StopPct = ' num2str(s) ', NPWDD = ' num2str(a)]);

tradesLimit=100;
[a,x1]=(max(-resultInSample(:,:,1)./resultInSample(:,:,2).*(resultInSample(:,:,4)<tradesLimit)));
[a,x2]=max(a); 
x1=x1(x2);
disp(['optimal length: ' num2str(Length(x1)) ', optimal stopPct: ' num2str(StopPct(x2))]);
figure(4);heatmap(-resultInSample(:,:,1)./resultInSample(:,:,2).*(resultInSample(:,:,4)<tradesLimit));
diary N1.txt